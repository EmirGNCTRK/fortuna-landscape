 ...
 * Adım 1: Temel Kurulumlar ve Hero Bölümü Yönetimi
 */

// 1. TEMA KURULUMU VE DESTEKLERİ
function fortunalandscape_setup() {
    // Sayfa başlıklarını dinamik yapar
    add_theme_support('title-tag');
    
    // Öne çıkarılan görsel (Kapak fotoğrafı) desteği
    add_theme_support('post-thumbnails');
    
    // Özel logo desteği
    add_theme_support('custom-logo');
    
    // Menü konumu tanımlama
...
// 3. TEMA ÖZELLEŞTİRİCİ (CUSTOMIZER) - HERO BÖLÜMÜ DÜZENLEYİCİSİ
function fortunalandscape_hero_customizer($wp_customize) {
    
    // Hero Düzenleyici Sekmesini Panelde Oluştur
    $wp_customize->add_section('hero_section', array(
        'title'    => __('Hero (Giriş) Düzenleyici', 'fortunalandscape'),
        'priority' => 10,
    ));
